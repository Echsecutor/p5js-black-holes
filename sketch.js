...
function control () {
  ...
  if (touches.length > 0) {
    ...
    player.object.acceleration = p5.Vector.sub(
  ...
  for (var i = 0; i < objects.length; i++) {
    ...
    for (var j = i + 1; j < objects.length; j++) {
      if (objects[i].object.collides_with(objects[j].object)) {
        ...
        var direction = p5.Vector.sub(
          objects[i].object.position,
          objects[j].object.position
        )
        var dist = direction.mag() + 0.01

        objects[i].object.acceleration = p5.Vector.mult(
          direction,
          (-gravity_const * objects[j].object.mass) / dist / dist
        )
        objects[j].object.acceleration = p5.Vector.mult(
          direction,
          (gravity_const * objects[i].object.mass) / dist / dist
        )
      }
    }
  }
...
function draw () {
  ...
  textSize(16)
  text(
    'Difficulty: ' + Math.round(100 * difficulty),
    10,
    10,
    width - 10,
    height - 10
